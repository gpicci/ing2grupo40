...
function cantCalifPendPiloto ($usuario_id, $diasLimite) {
    $db = DB::singleton();
    // valido que el piloto haya calificado pasajeros (pasajero.usuario_id in calificacion)
    $query = "
     SELECT COUNT(1) cant
     FROM (
...
function cantCalifPendCopiloto ($usuario_id, $diasLimite) {
    $db = DB::singleton();
    // valido que el copiloto haya calificado al piloto (viaje.usuario_id in calificacion)
    $query = "
     SELECT COUNT(1) cant
     FROM (
     SELECT p.viaje_id, p.usuario_id
...
function existeCalificacion($usuario_evalua_id, $usuario_evaluado_id, $viaje_id) {
    ...
    $query = "
                ...
                FROM calificacion
                WHERE
                usuario_evalua_id = $usuario_evalua_id
                AND usuario_evaluado_id = $usuario_evaluado_id
